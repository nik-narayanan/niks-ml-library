...
TEST(lu_decomposition_tests, lu_decomposition_small)
{
    ...
    auto request = LUDecomposition::Request(test_span);
    auto required_memory = LUDecomposition::required_memory(request);
    auto memory = MemoryOwner(required_memory.total_bytes());

    {
        auto lu_result = LUDecomposition::decompose(request, memory.to_request_memory(required_memory));
        ASSERT_TRUE(lu_result.is_ok());
        auto lu = lu_result.ok();
        lu.l.multiply(lu.u, reconstruct_span);
        ASSERT_TRUE(reconstruct_span.equals(test_span, 1e-06));
    }
}

TEST(lu_decomposition_tests, lu_decomposition_medium)
{
...
TEST(lu_decomposition_tests, lu_decomposition_large)
{
...
TEST(lu_decomposition_tests, lu_decomposition_iris)
{
    MatrixOwner test_matrix = MatrixOwner::from_delimited(TestData::test_file_path(Dataset::IRIS_VAL));
    MatrixOwner reconstruct = MatrixOwner(test_matrix.row_ct, test_matrix.column_ct);
